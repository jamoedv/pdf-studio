...
function LoginScreen({ onLogin, onCancel }) {
  ...
  const submit = async () => {
    if (!username.trim() || !password.trim()) {
      setError('Bitte Nutzername und Passwort eingeben.');
      return;
    }
    setLoading(true);
    setError('');
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <div className="space-y-3">
            <div>
              <label className="text-xs text-slate-500 mb-1.5 block">Nutzername</label>
              <input
                type="text"
                value={username}
            ...
            <div>
              <label className="text-xs text-slate-500 mb-1.5 block">Passwort</label>
              <input
                type="password"
                value={password}
                ...
                className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-200"
              />
              {mode === 'register' && <p className="text-xs text-slate-400 mt-1">Mindestens 8 Zeichen</p>}
            </div>
            ...
            <button
              onClick={submit}
              disabled={loading}
              className="w-full py-2.5 bg-blue-900 text-white rounded-lg text-sm font-medium hover:bg-blue-950 disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : (mode === 'login' ? 'Anmelden' : 'Konto erstellen')}
            </button>
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  const [complianceReportPath, setComplianceReportPath] = useState(null);
  ...
  const saveCurrentAsTemplate = async () => {
    ...
    try {
      await authFetch(`${API_URL}/templates`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const changeUserRole = async (username, role) => {
    try {
      const res = await authFetch(`${API_URL}/auth/users/${username}/role`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const deleteUserAccount = async (username) => {
    try {
      const res = await authFetch(`${API_URL}/auth/users/${username}`, { method: 'DELETE' });
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.error || 'Fehler');
      }
  ...
  const createUserAdmin = async () => {
    ...
    try {
      ...
      if (newUserRole === 'admin') {
        await authFetch(`${API_URL}/auth/users/${newUsername}/role`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
  ...
  const processWorkflow = async () => {
    ...
    try {
      ...
      const response = await authFetch(`${API_URL}/workflow`, { method: 'POST', body: formData });
  ...
  const processFiles = async () => {
    ...
    try {
      ...
      const response = await authFetch(`${API_URL}${endpoint}`, { method: 'POST', body: formData });
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const interval = setInterval(async () => {
          try {
            const res = await authFetch(`${API_URL}/job/${jobData.jobId}`);
            const data = await res.json();
            if (data.status === 'completed') {
  ...
  const processCompliance = async () => {
    ...
    try {
      ...
      const response = await authFetch(`${API_URL}/compliance-check`, { method: 'POST', body: formData });
      const data = await response.json();
      ...
      setComplianceReportPath(data.outputPath || null);
    } catch (err) {
      setComplianceError(err.message || 'Ein Fehler ist aufgetreten');
    }
      ...
      <div className="flex h-screen">
          ...
          <div className="max-w-4xl mx-auto p-8">
                    ...
                    {historyEntries.map(entry => (
                      <div key={entry.id + entry.timestamp} className="flex items-center justify-between p-2 bg-slate-50 rounded-lg text-xs">
                        <div className="min-w-0">
                          ...
                          <p className="text-slate-400">{new Date(entry.timestamp).toLocaleString('de-DE')}{entry.username ? ` · ${entry.username}` : ''}</p>
                        </div>
                        {entry.outputPath && (
                          <button
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-3 mb-5">
                      <div className="bg-slate-50 rounded-lg p-3">
                        <p className="text-xs text-slate-400 mb-1">Verarbeitungen gesamt</p>
                        <p className="text-2xl font-semibold text-slate-900">{statsData.totalJobs}</p>
                      </div>
                      <div className="bg-slate-50 rounded-lg p-3">
                        <p className="text-xs text-slate-400 mb-1">Speicher gespart</p>
                        <p className="text-2xl font-semibold text-emerald-600">{statsData.totalStorageSavedMB} MB</p>
                      </div>
                    </div>

                    {statsData?.daily?.length > 0 && (
                      <div className="mb-5">
                        <p className="text-xs font-medium text-slate-500 mb-2">Letzte 14 Tage</p>
                        <div className="flex items-end gap-1 h-20">
                          {statsData?.daily?.map((d, i) => {
                            const max = Math.max(...statsData.daily.map(x => x.count), 1);
                            const heightPct = (d.count / max) * 100;
                            return (
                              <div key={i} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                                <div
                                  className="w-full bg-blue-900 rounded-sm hover:bg-blue-950 transition-colors"
                    ...
                    {statsData?.byType?.length > 0 && (
                      <div className="mb-5">
                        <p className="text-xs font-medium text-slate-500 mb-2">Nach Funktion</p>
                        <div className="space-y-1.5">
                          {statsData?.byType?.slice(0, 6).map((t, i) => {
                            const max = statsData.byType[0].count;
                            return (
                              <div key={i} className="flex items-center gap-2">
                                <span className="text-xs text-slate-600 w-28 truncate">{t.type}</span>
                                <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                                  <div className="h-full bg-blue-900 rounded-full" style={{ width: `${(t.count / max) * 100}%` }} />
                                </div>
                    ...
                    {currentUser?.role === 'admin' && statsData?.byUser?.length > 0 && (
                      <div>
                        <p className="text-xs font-medium text-slate-500 mb-2">Nach Nutzer</p>
                        <div className="space-y-1.5">
                          {statsData.byUser.map((u, i) => (
                            <div key={i} className="flex items-center justify-between bg-slate-50 rounded-lg px-3 py-1.5">
                              ...
                              <span className="text-xs font-medium text-slate-700">{u.count}</span>
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              <div className="max-w-3xl bg-white border border-slate-200 rounded-xl p-5">
                ...
                {adminError && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-2.5 mb-3 flex items-center gap-2">
                    <AlertCircle className="w-3.5 h-3.5 text-red-500 flex-shrink-0" />
                    <p className="text-xs text-red-700">{adminError}</p>
                  </div>
                )}

                <div className="bg-slate-50 rounded-lg p-3 mb-4">
                  <p className="text-xs font-medium text-slate-500 mb-2">Neuen Nutzer anlegen</p>
                  ...
                  <div className="flex gap-2">
                    <select
                      ...
                      className="px-2.5 py-1.5 bg-white border border-slate-200 rounded-md text-xs outline-none"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 px-3 py-1.5 bg-blue-900 text-white rounded-md text-xs font-medium hover:bg-blue-950"
                    >
                      Anlegen
                    </button>
                  </div>
            ...
            {activeTab === 'compliance' && (
              <div className="bg-white border border-slate-200 rounded-xl p-4 mb-4">
                ...
                <p className="text-xs text-slate-400 mt-2">Unten die zu prüfenden Prüfberichte hochladen (mehrere möglich)</p>
              </div>
            )}
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 mb-4">
              ...
              >
                ...
                <input
                  ...
                  multiple={activeTab === 'merge' || (activeTab === 'convert' && convertDirection === 'toPDF') || ['extract-tables', 'extract-keyvalues', 'extract-standards', ...SINGLE_FILE_TABS].includes(activeTab) || activeTab === 'compliance'}
                ...
                {files.map((file, i) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-lg">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      {files.length > 1 && (
                        <>
                          <button onClick={() => moveFile(i, -1)} disabled={i === 0} className="p-1.5 hover:bg-slate-100 rounded-md transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
                            <ArrowUp className="w-3.5 h-3.5 text-slate-400" />
                          </button>
                          <button onClick={() => moveFile(i, 1)} disabled={i === files.length - 1} className="p-1.5 hover:bg-slate-100 rounded-md transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
            ...
            {activeTab === 'split' && files.length > 0 && (
              <div className="mb-4 bg-white border border-slate-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <button onClick={() => setSplitMode('every')} className={`py-2.5 rounded-lg text-xs font-medium transition-colors border ${splitMode === 'every' ? 'bg-blue-900 text-white border-blue-900' : 'bg-white text-slate-500 hover:bg-slate-50 border-slate-200'}`}>
                    Alle X Seiten
                  </button>
                  <button onClick={() => setSplitMode('ranges')} className={`py-2.5 rounded-lg text-xs font-medium transition-colors border ${splitMode === 'ranges' ? 'bg-blue-900 text-white border-blue-900' : 'bg-white text-slate-500 hover:bg-slate-50 border-slate-200'}`}>
                    Bestimmte Seiten
                  </button>
                </div>
                {splitMode === 'every' ? (
                  <div>
                    ...
                    <input type="number" min="1" value={pagesPerSplit} onChange={(e) => setPagesPerSplit(e.target.value)} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-200" />
                  ...
                  <div>
                    ...
                    <input type="text" value={splitRanges} onChange={(e) => setSplitRanges(e.target.value)} placeholder="z.B. 1-3,5,8-10" className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-200" />
            ...
            {activeTab === 'convert' && (
              <div className="mb-4 bg-white border border-slate-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => { setConvertDirection('toPDF'); setFiles([]); }} className={`py-2.5 rounded-lg text-xs font-medium transition-colors border ${convertDirection === 'toPDF' ? 'bg-blue-900 text-white border-blue-900' : 'bg-white text-slate-500 hover:bg-slate-50 border-slate-200'}`}>
                    Bilder → PDF
                  </button>
                  <button onClick={() => { setConvertDirection('toImages'); setFiles([]); }} className={`py-2.5 rounded-lg text-xs font-medium transition-colors border ${convertDirection === 'toImages' ? 'bg-blue-900 text-white border-blue-900' : 'bg-white text-slate-500 hover:bg-slate-50 border-slate-200'}`}>
                    PDF → Bilder
                  </button>
                </div>
              </div>
            ...
            {activeTab === 'rotate' && files.length > 0 && (
              <div className="mb-4 bg-white border border-slate-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[90, 180, 270].map(deg => (
                    <button key={deg} onClick={() => setRotateDegrees(deg)} className={`py-2.5 rounded-lg text-xs font-medium transition-colors border flex items-center justify-center gap-1.5 ${rotateDegrees === deg ? 'bg-blue-900 text-white border-blue-900' : 'bg-white text-slate-500 hover:bg-slate-50 border-slate-200'}`}>
            ...
            {activeTab === 'watermark' && files.length > 0 && (
              <div className="mb-4 bg-white border border-slate-200 rounded-xl p-4">
                ...
                <input type="text" value={watermarkText} onChange={(e) => setWatermarkText(e.target.value)} placeholder="z.B. ENTWURF, VERTRAULICH" className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-200" />
            ...
            {activeTab === 'password' && (
              <div className="mb-4 bg-white border border-slate-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <button onClick={() => setPasswordMode('set')} className={`py-2.5 rounded-lg text-xs font-medium transition-colors border ${passwordMode === 'set' ? 'bg-blue-900 text-white border-blue-900' : 'bg-white text-slate-500 hover:bg-slate-50 border-slate-200'}`}>
                    Passwort setzen
                  </button>
                  <button onClick={() => setPasswordMode('remove')} className={`py-2.5 rounded-lg text-xs font-medium transition-colors border ${passwordMode === 'remove' ? 'bg-blue-900 text-white border-blue-900' : 'bg-white text-slate-500 hover:bg-slate-50 border-slate-200'}`}>
                    Passwort entfernen
                  </button>
                </div>
                <label className="text-xs text-slate-500 mb-1.5 block">{passwordMode === 'set' ? 'Neues Passwort' : 'Aktuelles Passwort'}</label>
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Passwort eingeben" className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-200" />
              </div>
            )}
            ...
            {activeTab === 'metadata' && files.length > 0 && (
              <div className="mb-4 bg-white border border-slate-200 rounded-xl p-4 space-y-3">
                <div>
                  ...
                  <input type="text" value={metaTitle} onChange={(e) => setMetaTitle(e.target.value)} placeholder="z.B. Jahresbericht 2026" className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-200" />
                </div>
                <div>
                  <label className="text-xs text-slate-500 mb-1.5 block">Autor</label>
                  <input type="text" value={metaAuthor} onChange={(e) => setMetaAuthor(e.target.value)} placeholder="z.B. Max Mustermann" className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-200" />
                </div>
                <div>
                  <label className="text-xs text-slate-500 mb-1.5 block">Betreff</label>
                  <input type="text" value={metaSubject} onChange={(e) => setMetaSubject(e.target.value)} placeholder="z.B. Quartalszahlen" className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-200" />
                </div>
              </div>
            )}
            ...
            {activeTab === 'ocr' && files.length > 0 && (
              <div className="mb-4 bg-white border border-slate-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[{ value: 'deu+eng', label: 'DE + EN' }, { value: 'deu', label: 'Deutsch' }, { value: 'eng', label: 'English' }].map(opt => (
                    <button key={opt.value} onClick={() => setOcrLanguage(opt.value)} className={`py-2.5 rounded-lg text-xs font-medium transition-colors border ${ocrLanguage === opt.value ? 'bg-blue-900 text-white border-blue-900' : 'bg-white text-slate-500 hover:bg-slate-50 border-slate-200'}`}>
                      {opt.label}
                    </button>
                  ))}
                </div>
            ...
            {activeTab === 'redact' && files.length > 0 && (
              <div className="mb-4 bg-white border border-slate-200 rounded-xl p-4">
                <label className="flex items-center gap-2.5 cursor-pointer mb-3">
                  <input type="checkbox" checked={redactUseAI} onChange={(e) => setRedactUseAI(e.target.checked)} className="w-4 h-4 accent-blue-900" />
                  <div>
            ...
            {['extract-keyvalues', 'extract-standards', 'compare'].includes(activeTab) && (
              <label className="flex items-center gap-2.5 mb-4 bg-white border border-slate-200 rounded-xl p-4 cursor-pointer">
                <input type="checkbox" checked={generateReport} onChange={(e) => setGenerateReport(e.target.checked)} className="w-4 h-4 accent-blue-900" />
                <div>
            ...
            {activeTab === 'compliance' && (
              <>
                <label className="flex items-center gap-2.5 mb-4 bg-white border border-slate-200 rounded-xl p-4 cursor-pointer">
                  <input type="checkbox" checked={generateReport} onChange={(e) => setGenerateReport(e.target.checked)} className="w-4 h-4 accent-blue-900" />
                  <div>
                    <p className="text-sm font-medium text-slate-700">PDF-Sammelbericht erstellen</p>
                    <p className="text-xs text-slate-400">Ein Dokument mit allen Prüfergebnissen zum Herunterladen</p>
                  </div>
                </label>

                <button
                  onClick={processCompliance}
                  disabled={complianceLoading || !referenceFile || files.length === 0}
                ...
                >
                  {complianceLoading ? (<><Loader2 className="w-4 h-4 animate-spin" />Wird geprüft...</>) : (<><ClipboardCheck className="w-4 h-4" />Prüfung starten</>)}
                </button>
                ...
                {complianceResults && (
                  <div className="space-y-2 mb-4">
                    {complianceReportPath && (
                      <button onClick={() => downloadFile(complianceReportPath, 'compliance_bericht.pdf')} className="w-full py-2.5 bg-blue-900 text-white rounded-lg text-sm font-medium hover:bg-blue-950 transition-colors flex items-center justify-center gap-2 mb-2">
                        <Download className="w-4 h-4" />
                        Sammelbericht herunterladen
                      </button>
                    )}
                    {complianceResults.map((r, i) => {
                      const isExpanded = expandedReport === i;
                      const badgeStyle = r.overallResult === 'bestanden' ? 'bg-emerald-50 text-emerald-700' : r.overallResult === 'nicht_bestanden' ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700';
                      const badgeLabel = r.overallResult === 'bestanden' ? 'Bestanden' : r.overallResult === 'nicht_bestanden' ? 'Nicht bestanden' : 'Unklar';
                      return (
                        <div key={i} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                          <button onClick={() => setExpandedReport(isExpanded ? null : i)} className="w-full flex items-center justify-between p-4 hover:bg-slate-50 transition-colors">
                            <span className="text-sm font-medium text-slate-700 truncate">{r.filename}</span>
                            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full flex-shrink-0 ml-3 ${badgeStyle}`}>{badgeLabel}</span>
                          </button>
                          {isExpanded && (
                            <div className="px-4 pb-4 border-t border-slate-100 pt-3">
                              {r.summary && <p className="text-xs text-slate-600 mb-3 leading-relaxed">{r.summary}</p>}
                              {r.note && <p className="text-xs text-amber-600 mb-3">{r.note}</p>}
                              {r.criteria && r.criteria.length > 0 && (
                                <div className="space-y-1.5">
                                  {r.criteria.map((c, ci) => (
                                    <div key={ci} className="flex items-center justify-between gap-2 py-1.5 border-b border-slate-50 last:border-0">
                                      <div className="min-w-0 flex-1">
                                        <p className="text-xs font-medium text-slate-700 truncate">{c.name}</p>
                                        <p className="text-xs text-slate-400">Soll: {c.required} · Ist: {c.actual}</p>
                                      </div>
                                      <span className={`text-xs px-2 py-0.5 rounded-md flex-shrink-0 ${c.result === 'bestanden' ? 'bg-emerald-50 text-emerald-700' : c.result === 'nicht_bestanden' ? 'bg-red-50 text-red-700' : 'bg-slate-100 text-slate-500'}`}>
                                        {c.result === 'bestanden' ? '✓' : c.result === 'nicht_bestanden' ? '✗' : '?'}
                                      </span>
                                    </div>
                                  ))}
                                </div>
            ...
            {(['extract-tables', 'extract-keyvalues', 'extract-standards', 'compare', 'summarize'].includes(activeTab)) && (
              <>
                <button
                  ...
                  className="w-full py-3 rounded-lg font-medium text-sm text-white bg-blue-900 hover:bg-blue-950 disabled:bg-slate-300 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2 mb-4"
                >
                  {extractionLoading ? (<><Loader2 className="w-4 h-4 animate-spin" />Wird analysiert...</>) : (<><Check className="w-4 h-4" />Analysieren</>)}
                </button>
                      ...
                      <div key={ri} className="bg-white border border-slate-200 rounded-xl p-5">
                        ...
                        {r.error ? (
                          ...
                          <>
                            ...
                            {r.outputPath && (
                              <button onClick={() => downloadFile(r.outputPath, buildDownloadName(r.filename, 'kennwerte_bericht'))} className="w-full py-2.5 bg-blue-900 text-white rounded-lg text-sm font-medium hover:bg-blue-950 transition-colors flex items-center justify-center gap-2">
                                <Download className="w-4 h-4" />
                      ...
                      <div key={ri} className="bg-white border border-slate-200 rounded-xl p-5">
                        ...
                        {r.error ? (
                          ...
                          <>
                            ...
                            {r.outputPath && (
                              <button onClick={() => downloadFile(r.outputPath, buildDownloadName(r.filename, 'normen_bericht'))} className="w-full py-2.5 bg-blue-900 text-white rounded-lg text-sm font-medium hover:bg-blue-950 transition-colors flex items-center justify-center gap-2">
                                <Download className="w-4 h-4" />
                                PDF-Bericht herunterladen
                              </button>
                            )}
                      ...
                      <div key={ri} className="bg-white border border-slate-200 rounded-xl p-5">
                        ...
                        {r.error ? (
                          ...
                          <>
                            ...
                            <button onClick={() => downloadFile(r.outputPath, buildDownloadName(r.filename, 'tabellen', 'xlsx'))} className="w-full py-2.5 bg-blue-900 text-white rounded-lg text-sm font-medium hover:bg-blue-950 transition-colors flex items-center justify-center gap-2">
                              <Download className="w-4 h-4" />
                  ...
                  <div className="bg-white border border-slate-200 rounded-xl p-5">
                    ...
                    <div className="max-h-64 overflow-y-auto bg-slate-50 rounded-lg p-3 text-xs leading-relaxed mb-4">
                      {extractionResult.segments.map((seg, i) => (
                        <span key={i} className={seg.type === 'added' ? 'bg-emerald-100 text-emerald-800' : seg.type === 'removed' ? 'bg-red-100 text-red-800 line-through' : 'text-slate-500'}>
                          {seg.value}
                        </span>
                      ))}
                    </div>
                    {extractionResult.outputPath && (
                      <button onClick={() => downloadFile(extractionResult.outputPath, 'vergleichsbericht.pdf')} className="w-full py-2.5 bg-blue-900 text-white rounded-lg text-sm font-medium hover:bg-blue-950 transition-colors flex items-center justify-center gap-2">
                        <Download className="w-4 h-4" />
                        PDF-Bericht herunterladen
                      </button>
                    )}
            ...
            {pendingSteps && (
              <div className="mb-4 bg-blue-900 text-white rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={() => setShowSaveTemplate(prev => !prev)} className="text-xs text-slate-400 hover:text-white flex items-center gap-1">
                ...
                <div className="space-y-2 mb-1">
                  {pendingSteps.map((s, i) => (
                    <div key={i} className="flex items-center gap-2 flex-wrap">
                      ...
                      {s.action === 'compress' && (
                        <select value={s.options?.level || 'medium'} onChange={(e) => updateStepOption(i, 'level', e.target.value)} className="bg-white/10 text-white text-xs rounded-md px-2 py-1 outline-none border-none">
                      ...
                      {s.action === 'rotate' && (
                        <select value={s.options?.degrees || 90} onChange={(e) => updateStepOption(i, 'degrees', parseInt(e.target.value))} className="bg-white/10 text-white text-xs rounded-md px-2 py-1 outline-none border-none">
                      ...
                      {s.action === 'watermark' && (
                        <input type="text" value={s.options?.text || ''} onChange={(e) => updateStepOption(i, 'text', e.target.value)} placeholder="Wasserzeichen-Text" className="bg-white/10 text-white text-xs rounded-md px-2 py-1 outline-none placeholder-slate-500 w-36" />
                      )}
                      {s.action === 'password' && (
                        <>
                          <select value={s.options?.mode || 'set'} onChange={(e) => updateStepOption(i, 'mode', e.target.value)} className="bg-white/10 text-white text-xs rounded-md px-2 py-1 outline-none border-none">
                            <option className="text-slate-900" value="set">Setzen</option>
                            <option className="text-slate-900" value="remove">Entfernen</option>
                          </select>
                          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Passwort" className="bg-white/10 text-white text-xs rounded-md px-2 py-1 outline-none placeholder-slate-500 w-28" />
                        </>
                      )}
                      {s.action === 'convert' && (
                        <select value={s.options?.direction || 'toPDF'} onChange={(e) => updateStepOption(i, 'direction', e.target.value)} className="bg-white/10 text-white text-xs rounded-md px-2 py-1 outline-none border-none">
                      ...
                      {s.action === 'split' && (
                        <>
                          <select value={s.options?.mode || 'every'} onChange={(e) => updateStepOption(i, 'mode', e.target.value)} className="bg-white/10 text-white text-xs rounded-md px-2 py-1 outline-none border-none">
                          ...
                          {(s.options?.mode || 'every') === 'every' ? (
                            <input type="number" min="1" value={s.options?.pagesPerSplit || 1} onChange={(e) => updateStepOption(i, 'pagesPerSplit', parseInt(e.target.value))} className="bg-white/10 text-white text-xs rounded-md px-2 py-1 outline-none w-16" />
                          ) : (
                            <input type="text" value={s.options?.ranges || ''} onChange={(e) => updateStepOption(i, 'ranges', e.target.value)} placeholder="1-3,5" className="bg-white/10 text-white text-xs rounded-md px-2 py-1 outline-none placeholder-slate-500 w-24" />
                          )}
                        </>
                      )}
                      {s.action === 'metadata' && (
                        <>
                          <input type="text" value={s.options?.title || ''} onChange={(e) => updateStepOption(i, 'title', e.target.value)} placeholder="Titel" className="bg-white/10 text-white text-xs rounded-md px-2 py-1 outline-none placeholder-slate-500 w-28" />
                          <input type="text" value={s.options?.author || ''} onChange={(e) => updateStepOption(i, 'author', e.target.value)} placeholder="Autor" className="bg-white/10 text-white text-xs rounded-md px-2 py-1 outline-none placeholder-slate-500 w-28" />
                        </>
                      )}
                      {s.action === 'ocr' && (
                        <select value={s.options?.language || 'deu+eng'} onChange={(e) => updateStepOption(i, 'language', e.target.value)} className="bg-white/10 text-white text-xs rounded-md px-2 py-1 outline-none border-none">
                ...
                {showSaveTemplate && (
                  <div className="flex items-center gap-2 mt-3">
                    <input type="text" value={saveTemplateName} onChange={(e) => setSaveTemplateName(e.target.value)} placeholder="Name der Vorlage" className="flex-1 px-2.5 py-1.5 bg-white/10 rounded-md text-xs outline-none placeholder-slate-500 focus:ring-1 focus:ring-white/30" />
                    <button onClick={saveCurrentAsTemplate} disabled={!saveTemplateName.trim()} className="px-3 py-1.5 bg-white text-slate-900 rounded-md text-xs font-medium disabled:opacity-40">
                      Speichern
                    </button>
                  </div>
                )}
            ...
            {files.length > 1 && SINGLE_FILE_TABS.includes(activeTab) && !pendingSteps && (
              <>
                <button
                  ...
                  className="w-full py-3 rounded-lg font-medium text-sm text-white bg-blue-900 hover:bg-blue-950 disabled:bg-slate-300 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2 mb-4"
                >
                  {batchProcessing ? (<><Loader2 className="w-4 h-4 animate-spin" />Datei {batchProgress.current} von {batchProgress.total}...</>) : (<><Check className="w-4 h-4" />Alle {files.length} Dateien verarbeiten</>)}
                </button>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="space-y-2">
                      {batchResults.map((r, i) => (
                        <label key={i} className={`flex items-center gap-3 bg-white border rounded-lg p-3.5 transition-colors ${r.success ? 'cursor-pointer border-slate-200 hover:border-slate-300' : 'border-red-100'}`}>
                          {r.success ? (
                            <input type="checkbox" checked={!!selectedBatchFiles[i]} onChange={() => toggleBatchFileSelection(i)} className="w-4 h-4 accent-blue-900 flex-shrink-0" />
                          ) : (
                            <AlertCircle className="w-4 h-4 text-red-500 flex-shrink-0" />
                          )}
                          <div className="min-w-0 flex-1">
            ...
            {files.length > 0 && !['extract-tables', 'extract-keyvalues', 'extract-standards', 'compare', 'summarize', 'compliance'].includes(activeTab) && !(files.length > 1 && SINGLE_FILE_TABS.includes(activeTab) && !pendingSteps) && (
              <button
              ...
              >
                {processing ? (<><Loader2 className="w-4 h-4 animate-spin" />Wird verarbeitet...</>) : pendingSteps ? (<><Check className="w-4 h-4" />Workflow starten ({pendingSteps.length} Schritte)</>) : (<><Check className="w-4 h-4" />Datei verarbeiten</>)}
              </button>
            )}

            {pendingSteps && !processing && (
              <button onClick={cancelWorkflow} className="w-full py-2 rounded-lg font-medium text-xs text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors mb-4">
                Workflow abbrechen
              </button>
            )}
                      ...
                      <div key={i} className="flex items-center justify-between bg-slate-50 rounded-lg px-3 py-2 text-xs">
                        ...
                        <span className="text-slate-500">
                          {step.action === 'compress' && step.savings !== undefined && (
                            step.savings > 0 ? `${formatFileSize(step.originalSize)} → ${formatFileSize(step.compressedSize)} (−${step.savings}%)` : 'keine Einsparung möglich'
                          )}
                ...
                {result.files && result.files.length > 0 ? (
                  <div className="space-y-2">
                    {result.files.map((filePath, i) => {
                      ...
                      return (
                        <button key={i} onClick={() => downloadFile(filePath)} className="w-full py-2.5 px-3 bg-blue-900 text-white rounded-lg text-sm font-medium hover:bg-blue-950 transition-colors flex items-center justify-between gap-2">
        ...
        {chatCollapsed ? (
          <div className="w-14 border-l border-slate-200 bg-white flex flex-col items-center pt-4">
            <button onClick={() => setChatCollapsed(false)} className="w-9 h-9 rounded-lg flex items-center justify-center text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors">
        ...
        <div className="w-96 border-l border-slate-200 bg-white flex flex-col">
          <div className="p-4 border-b border-slate-200 flex items-center justify-between">
            ...
            <button onClick={() => setChatCollapsed(true)} className="w-7 h-7 rounded-md flex items-center justify-center text-slate-400 hover:bg-slate-100 transition-colors">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] px-3.5 py-2.5 rounded-xl text-sm leading-relaxed ${msg.role === 'user' ? 'bg-blue-900 text-white rounded-br-sm' : 'bg-white border border-slate-200 text-slate-700 rounded-bl-sm'}`}>
                  {msg.text}
                </div>
              </div>
            ))}
